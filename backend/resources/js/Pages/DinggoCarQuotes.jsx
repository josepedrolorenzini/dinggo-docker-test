...
function DinggoCarQuotes({ auth, car, quotes, message, success }) {
    const [carData, setCarData] = useState(car);
    const [quotesData, setQuotesData] = useState(quotes);

    useEffect(() => {
        if (car) setCarData(car);
        if (quotes) setQuotesData(quotes);
        console.log(car);
        console.log(quotes);
    }, [car, quotes]);
    ...
    return (
        <>
            <Head title={`Quotes for ${car?.licensePlate}`} />
            <div className="bg-gray-50 text-black/50 dark:bg-black dark:text-white/50">
                <div className="relative flex min-h-screen flex-col items-center justify-center selection:bg-[#FF2D20] selection:text-white">
                    <div className="relative w-full max-w-2xl px-6 lg:max-w-7xl">
                        ...
                        {/* Main Content */}
                        <main className="mt-6 w-full max-w-3xl">
                                        ...
                                        <ul style={{ margin: 0, padding: 0 }}>
                                            {quotesData?.quotes?.length > 0 ? (
                                                quotesData.quotes.map((quote, i) => (
                                                    <div style={boxStyle}>
                                                        <li key={i}>
                                                            <strong>{quote.repairer}</strong>: ${quote.price} <br />
                                                            {quote.overviewOfWork}
                                                        </li>
                                                    </div>
                                                ))
                                            ) : (
...
export default DinggoCarQuotes;
